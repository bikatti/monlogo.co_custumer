...
function mostrarPestana(n){
    ...
    for(i = 0; i<pestanas.length; i++){
        if(pestanas[i].className.includes("-isActiveTab")){
            pestanas[i].className = pestanas[i].className.replace("-isActiveTab", "");
            cabecera[i].className = cabecera[i].className.replace("-isActiveTab", "");
            break;
        }
    }
    pestanas[n].className += " -isActiveTab";
    cabecera[n].className += " -isActiveTab";
}
